...
const TaskList = () => {
    ...
    const handleInpuChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        // @ts-ignore
        setFilteredData(items.filter(item => item.name.toLowerCase().includes(e.target.value.toLowerCase())));
        handleTableData()
    };
    ...
    const onChangeSelect = () => {
        ...
        // @ts-ignore
        setFilteredData(() => items.filter(item => item.priority === selectRef?.current?.value));
        handleTableData()
    };
    ...
    const handleTableData = () => {
        ...
        const cols = [{data: 'ID', ratio: 2}, {data: "Job Name", ratio: 7}, {
            data: "Status",
            ratio: 4
        ...
        if ((inputRef.current && inputRef.current.value !== '') || selectRef.current && selectRef?.current?.value !== "Priority(All)") {
    ...
    const sortDataFunc = (field: string, state: React.ComponentState, setState: React.Dispatch<React.SetStateAction<any>>) => {
        if (state) {
    ...
    const getPriorities = () => {
        axios.get('http://localhost:5000').then(res => {
            setPritories(res.data.data.map((item: any)=> {
    ...
    return (
        <>
            ...
            <div className="background-container">
                <div className="group row">
                    <div className={'col-md-8'}>
                        <Input placeholder={"Job name"} ref={inputRef} onChange={handleInpuChange} value={inputValue}
                               isSearch={true}/>
                    </div>

                    <div className={'col-md-4'}>
                        <Select hasDefault={false} onChange={onChangeSelect} ref={selectRef}
                                options={[{
                                    value: 'Priority(All)',
                                    label: 'Priority(All)'
                                }, ...pritories]}/>
            ...
            {tableData &&
            <Table filterClick={sortData} data={tableData}/>

            }
            <PortalModal
                children={modalType === 'approve' ? <ApproveModal approve={deleteTask} toggle={toggle}
                                                                  content={'Are you sure you want to delete it?'}
                                                                  buttons={['Cancel', 'Approve']}/> :
                    <EditModal toggle={toggle} edit={editTask} selectedPriority={selectedItem[2]} jobName={'qwe'}/>}
                isOpen={modalOpen}
            />
